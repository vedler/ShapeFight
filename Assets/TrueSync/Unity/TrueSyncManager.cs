...
namespace TrueSync {
    ...
    public class TrueSyncManager : MonoBehaviour {
         ...
         **/
        public TSVector gravity = new TSVector(0, -10, 0);
        ...
        /**
         * @brief GameObjects that have attached scripts with {@link AddTracking} attribute.
         **/
        ...
        void Awake() {
            ...
            if (GameObject.FindObjectOfType<TSCollider2D>() != null) {
                PhysicsManager.New(PhysicsManager.PhysicsType.W_2D);
            } else {
                PhysicsManager.New(PhysicsManager.PhysicsType.W_3D);
            }

            PhysicsManager.instance.Gravity = gravity;
            PhysicsManager.instance.SpeculativeContacts = speculativeContacts;
        ...
        private void CheckTrackedGameObjects() {
            if (trackedGameObjects != null) {
                foreach (GameObject go in trackedGameObjects) {
                    foreach (Component comp in go.GetComponentsInChildren(typeof(MonoBehaviour))) {
                        StateTracker.AddTracking(comp);
                    }
                }
            }
        }
            ...
            if (instance != null && instance.lockstep != null && (prefab.GetComponentInChildren<TSTransform>() != null || prefab.GetComponentInChildren<TSTransform2D>() != null)) {
                ...
                if (tsTransforms2D != null) {
                    foreach (TSTransform2D tsTransform2D in tsTransforms2D) {
                        ...
                        tsTransform2D.position = new TSVector2(position.x, position.y);
                        tsTransform2D.rotation = rotation.ToQuaternion().eulerAngles.z;
                    }
                }
        ...
        public static void RemovePlayer(int playerId) {
            if (instance != null && instance.lockstep != null) {
                foreach (TrueSyncManagedBehaviour tsmb in instance.behaviorsByPlayer[(byte)playerId]) {
                    ...
                    Debug.Log(">> HAHAHA " + tsCollider2Ds.Length);
                    if (tsCollider2Ds != null) {
                        Debug.Log(">> " + tsCollider2Ds.Length);
